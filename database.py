...
def save_page(url, title, status, content_hash, html_path):
    ...
    if old:
        ...
        """, (title, status, content_hash, now, now, html_path, url))
    else:
        cursor.execute("""
        INSERT INTO pages(url,title,status,content_hash,crawl_time,update_time,html_path)
        VALUES(?,?,?,?,?,?,?)
...
def add_log(url, result, message):
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    cursor.execute("INSERT INTO logs(time,url,result,message) VALUES(?,?,?,?)",
